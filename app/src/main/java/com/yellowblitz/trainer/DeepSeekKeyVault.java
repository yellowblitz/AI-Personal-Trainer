...
/** Secure storage used only by the packaged trainer page for the DeepSeek API key. */
public final class DeepSeekKeyVault {
    private static final String ALIAS = "trainer.deepseek.key.v1";
    private final SharedPreferences prefs;
    public DeepSeekKeyVault(Context context) { prefs = context.getSharedPreferences("deepseek_vault", Context.MODE_PRIVATE); }
    ...
    @JavascriptInterface public synchronized String getKey() {
        try {
            String encrypted = prefs.getString("encrypted", "");
    ...
    @JavascriptInterface public synchronized boolean saveKey(String value) {
        ...
        try {
            ...
            return prefs.edit()
                .putString("encrypted", Base64.encodeToString(cipher.doFinal(value.getBytes(StandardCharsets.UTF_8)), Base64.NO_WRAP))
