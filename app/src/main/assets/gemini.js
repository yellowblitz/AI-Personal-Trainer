...
export function buildRequest({message,week,selectedDay,proposal=null,history=[],profile=null,memory='',recentTraining=[],preferences=[],model=DEFAULT_MODEL},catalog){
 ...
 const system=[
  ...
  'Use recentTraining as the strongest progression evidence. Each completed set may include plannedReps/plannedWeight and actualReps/actualWeight. Compare the prescription with what was really performed instead of assuming the target was completed. Exercise-level rir means reps in reserve: 0 means no more good reps were available; 1 means about one; 2-3 means some reserve; 4 means 4 or more. effort may be too_easy/right/too_hard, form may be good/breaking/poor, discomfort may be none/mild/pain, and note may add useful context. If actual performance is below target with RIR 0-1, form is breaking/poor, effort is too_hard, or discomfort was reported, do not blindly progress; normally hold, reduce, or propose an appropriate substitute. If pain is reported, do not diagnose it or prescribe rehabilitation; advise stopping painful activity and seeking appropriate assessment. If targets are consistently met with RIR 2-4, good form, no discomfort and manageable effort, a modest progression may be appropriate. Treat one unusual session cautiously and look for trends.',
  'exercisePreferences summarizes local behavior and may include a persistent per-exercise setup note. Repeated removals are a negative preference signal; repeated additions/completions are a positive preference signal. Respect useful setup notes when discussing or programming that exercise. Use these signals to avoid repeatedly prescribing movements the user tends to remove and to favor suitable movements they actually use, while still respecting the training goal, equipment, balance, recovery, and explicit user requests.',
