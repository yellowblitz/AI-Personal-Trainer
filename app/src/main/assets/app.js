...
$('settings').onclick=()=>{
 modal(`<h2>Coach & Gemini settings</h2><p class="muted">AI Personal Trainer · v0.4.0</p><h3>Training profile</h3><p class="small muted">This helps the coach tailor advice and plan drafts. Keep medical details out of this field.</p><label for="goal">Primary goal</label><select id="goal">${GOALS.map(g=>`<option value="${escape(g)}">${escape(g)}</option>`).join('')}</select><label for="experience">Training experience</label><select id="experience">${LEVELS.map(level=>`<option value="${escape(level)}">${escape(level)}</option>`).join('')}</select><label for="equipment">Equipment available</label><textarea id="equipment" rows="3" maxlength="500" placeholder="Example: functional trainer, adjustable dumbbells, stationary bike"></textarea><h3>Gemini connection</h3><label for="apiKey">Your Gemini API key</label><input id="apiKey" type="password" autocomplete="off" autocapitalize="off" spellcheck="false" maxlength="512" placeholder="Paste your Gemini API key"><div class="row"><button id="showKey" class="secondary">Show key</button><a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer">Get a Gemini API key ↗</a></div><button id="saveSettings" class="primary wide">Save settings</button><button id="clearKey" class="secondary wide">Remove key</button><p class="small muted">${window.TrainerKeys?'Your key is encrypted on this Android device and used only to contact Google Gemini.':'Browser preview: your key stays in memory until you reload or close this page.'}</p><p class="small muted">Gemini 2.5 Flash · No backend or Google login required here. Free-tier limits and any charges depend on your Google API project. Your training profile, recent chat, and weekly schedule are sent to Google when you message the coach; free-tier data may be used to improve Google products.</p><p class="small muted">Exercise photos: free-exercise-db (Unlicense). Rest alerts work while the app is active.</p>`);
 $('apiKey').value=apiKey;$('goal').value=profile.goal;$('experience').value=profile.experience;$('equipment').value=profile.equipment;
 ...
 $('saveSettings').onclick=()=>{const value=$('apiKey').value.trim();if(value&&/\s/.test(value))return toast('Paste a Gemini API key without spaces.');if(value!==apiKey&&!setKey(value))return;profile=normalizeProfile({goal:$('goal').value,experience:$('experience').value,equipment:$('equipment').value});saveProfile();$('modal').close();render();toast(value?'Settings saved. Your coach will use your training profile.':'Training profile saved. Add a Gemini key when you want AI coaching.');};
