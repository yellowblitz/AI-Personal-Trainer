...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#calendarGrid .calendar-cell.logged').count(),1);assert.equal(await page.locator('#weeklyReview').isVisible(),true);assert.equal(await page.locator('#muscleProgressCard').isVisible(),true);assert.ok((await page.locator('#muscleProgressList').textContent()).length>0);
