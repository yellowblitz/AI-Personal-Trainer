...
const input={message:'Change Monday reps',week:makeWeek(),selectedDay:'mon',profile,memory:'Prefers controlled progression.',model:DEFAULT_MODEL,recentTraining:[{date:'2026-09-19',day:'mon',exercises:[{id:'Pushups',sets:[{reps:12,weight:0},{reps:9,weight:0}]}]}]};
...
test('Gemini sends profile, memory, training history and fixed HTTPS key header',async()=>{const data=await askGemini(input,catalog,'my-private-key',async(url,opts)=>{assert.equal(url,GEMINI_URL);assert.equal(opts.headers['x-goog-api-key'],'my-private-key');assert.equal((url+opts.body).includes('my-private-key'),false);assert.equal(opts.redirect,'error');const body=JSON.parse(opts.body);assert.equal(body.generationConfig.responseMimeType,'application/json');assert.equal(body.generationConfig.thinkingConfig.thinkingLevel,'high');assert.equal('responseJsonSchema' in body.generationConfig,false);assert.match(body.systemInstruction.parts[0].text,/not a hard ceiling/);assert.match(body.systemInstruction.parts[0].text,/excluding warm-up/);const context=JSON.parse(body.contents[0].parts[0].text);assert.equal(context.userProfile.heightIn,70);assert.match(context.coachMemory,/controlled progression/);assert.equal(context.recentTraining[0].exercises[0].sets[1].reps,9);return {ok:true,json:async()=>result()};});assert.equal(data.action,'proposal');assert.equal(data.week.days.length,7);assert.match(data.memory,/fatigue/);});
